...
Auth::guard('web')->loginUsingId($id);
 ...
 //var_dump($user_id); 
  ...
  return redirect('/'.app()->getLocale() .'/system/account');
 ...
 public function editprofile ($id)
    {
       ...
       $user = User::find($id);
        ...
        return redirect('/'.app()->getLocale() .'/admin/users/')->with('status', 'Updated successfully');
...
$exchange_g2f = Exchange_g2f::all();
      ...
      public function getnokycall ()
    {
       

$u = User::where(function($query) {
        $query->where('kycstatus', 'no') ;
        })
        ->get();
        
      
        return view('backend.users.kycsno')
            ->with('users', $u);
 
    }
    
      public function getpendingkycall ()
    {
       

$u = User::where(function($query) {
        $query->where('kycstatus', 'Pending') ;
        })
        ->get();
        
      
        return view('backend.users.kycspending')
            ->with('users', $u);
 
    }
    
      public function getapprovedkycall ()
    {
       

$u = User::where(function($query) {
        $query->where('kycstatus', 'Approved') ;
        })
        ->get();
        
      
        return view('backend.users.kycsapproved')
            ->with('users', $u);
     ...
     public function g2fdelete($email )
    {
       ...
       // echo "";
        return redirect('/'.app()->getLocale() .'/admin/g2f')->with('status', 'Deleted for the username '.$email);
    }
