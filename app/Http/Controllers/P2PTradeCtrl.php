...
->where('status',  'Success')->firstOrFail();
 ...
 return back()->with('success',  'Message Sent Successfully' );
      ...
      public function submitsocketchat ( Request  $request)
    {
        ...
        $data->trade_order_id = $request->trade_order_id ;
    ...
    $response = array(
      ...
      public function socketchat ($id )
    {
        
        
        $flights = P2P_orders_discussion::where('trade_order_id', $id)
               ->orderBy('created_at', 'desc')
               ->take(10)
               ->get();
        ...
        {
            echo "<tr>";
            echo "<td>".$f->trade_message." ";
            
            echo "<br /><br />--".$f->username."</td>";
    echo "</tr>";
    }
  ...
  if($po->trade->trade_type=="ONLINE_BUY")
      {
           ...
           return redirect('/'.app()->getLocale() .'/p2p/process_trade_buy/'.$po->id) ;
        
      }
       if($po->trade->trade_type=="ONLINE_SELL")
      {
              
           return redirect('/'.app()->getLocale() .'/p2p/process_trade_sell/'.$po->id) ;
    }
...
->where('status',  'Active')->firstOrFail();
     ...
     if($save){ 
            ...
            return redirect('/'.app()->getLocale() .'/p2p/process_trade_sell/'.$data->id)->with('success','Your transaction started.');
...
->where('status',  'Active')->firstOrFail();
     ...
     if($save){ 
            ...
            return redirect('/'.app()->getLocale() .'/p2p/process_trade_buy/'.$data->id)->with('success','Your transaction started.');
    ...
    public function posttradesubmit(Request $request)
    {
        ...
        $save = $data->save();
        if($save){
            return redirect('/'.app()->getLocale() .'/home')->with('success','Your advertisement published successfully.');
        }
    }
