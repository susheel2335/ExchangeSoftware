...
class AuthController  extends Controller
{
 ...
 public function postkyc($id)
{
    
    $path = $this->request->file->storeAs('images',$id. 'filename.jpg');
     
  
    
   $img= base64_encode(file_get_contents($this->request->file ));
    
    $user = Auth::user();
    
    $k="kyc".$id;    
    
          $l="kyc_document_info".$id; 
...
else
{
    ...
    public function resetPassword()
    {
        ...
        if(isset($row->email))
        {
            ...
            return redirect()->route('frontend.homepage', app()->getLocale() )->with('status', 'failed')->with('message', trans('Your reset code is invalid or has expired.'));
        }
    }
    ...
    public function resetPasswordPost()
    {
        ...
        return redirect()->route('frontend.homepage',app()->getLocale()  )->with('status', 'success')->with('message', trans('auth.resetPasswordSuccess'));
    }
