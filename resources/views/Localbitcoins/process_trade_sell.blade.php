...
@endif
     ...
     <h1>  {{ __('p2p.contact') }} #{{ $po->id  }}:{{ __('p2p.sell') }}  {{ $po->btcvalue  }} {{ __('p2p.btcfor') }}  {{ $po->c1value  }}   {{ $po->currency  }} </h1> 

<h3>{{ __('p2p.sellinadvertisement') }} #{{ $po->trade_id  }} (Other online payment) to {{ $po->trade->username  }}{{ __('p2p.attheexchangerate') }} {{ $po->rate  }} {{ $po->currency  }} .</h3> 
...
{{ $po->btcvalue  }} {{ __('p2p.btcsenttobuyer ') }}{{ $po->created_at  }}
{{ __('p2p.xpagg.comfeebtc') }}: {{ $po->created_at  }}
